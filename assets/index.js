const FPS = 75; // Frames per Seecond ***Testar mais de 30 depois.
//
const LASER_MAX = 15; // Número máximo de "lasers" na tela ao mesmo tempo
const LASER_SPD = 700; // Velocidade dos "lasers" em pixels por segundo
const LASER_DIST = 0.6; // Distância máxima "lasers" podem viajar, fração da Screen Width
...
const TURN_SPEED = 150; // Velocidade de curvas, em graus por segundo.
//
...
let fxLaser = new Sound("sounds/laser.m4a", 15, 0.1);
let fxExplode = new Sound("sounds/explode.m4a", 1, 0.5);
let fxHit = new Sound("sounds/hit.m4a", 15, 0.3);
...
function Music(srcLow, srcHigh) {
  ...
  this.beatTime = 0; // frames que restam para proximo beat
  ...
  this.tick = function () {
    if (this.beatTime == 0) {
      this.play();
      this.beatTime = Math.ceil((this.tempo * FPS) / 1.2);
    } else {
      this.beatTime--;
    }
  };
...
let startB = document.querySelector("#startButton");
...
function destroyAsteroid(index) {
  ...
  // Calcular quantos asteroides ainda restam para determinar o tempo da musica
  roidsLeft--;
...
function createCircle() {
  ...
  for (let i = 0; i < CIRCLE_NUM + level; i++) {
    ...
    do {
      ...
      distBetweenPointsCircle(ship.x, ship.y, xc, yc) <
      CIRCLE_SIZE * 2 + ship.r
    );
...
function keyDown(/** @type {KeyboardEvent} */ ev) {
  ...
  switch (ev.keyCode) {
    // Apagar Best Score
    case 76:
      localStorage.removeItem("highscore"); // Remover valor de best score
      break;
...
function newAsteroid(x, y, r) {
  ...
  let roid = {
    ...
    y: y,
    xv:
      ((Math.random() * ROID_SPD * lvlMulti) / FPS) *
      (Math.random() < 0.5 ? 1 : -1),
    yv:
      ((Math.random() * ROID_SPD * lvlMulti) / FPS) *
      (Math.random() < 0.5 ? 1 : -1),
...
function shootLaser() {
  ...
  if (ship.canShoot && ship.lasers.length < LASER_MAX) {
    ship.lasers.push({
      // Atirar da parte da frente da nave
...
function update() {
  ...
  if (textAlpha >= 0) {
    ...
    context.font = "small-caps " + TEXT_SIZE * 1.3 + "px dejavu sans mono";
  ...
  context.fillStyle = "slategrey";
  context.font = TEXT_SIZE * 0.7 + "px dejavu sans mono";
